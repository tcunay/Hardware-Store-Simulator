...
namespace Gley.TrafficSystem
{
    public class MovementInfo
    {
        ...
        public void AddWaypointAsTarget(int waypointIndex, Vector3 waypointPosition, float waypointSpeed, bool stop, GiveWayType giveWayType, int angle)
        {
            if (waypointIndex < 0)
            {
                Debug.LogError($"Cannot add {waypointIndex.ToString()} as target");
                return;
            }
        ...
        public void SetClosestObstacleAndSpeed(Vector3 targetPosition)
        {
            if (HasObstacles())
            {
                ...
                if (_closestObstacle.ObstacleType == ObstacleTypes.Player || _closestObstacle.ObstacleType == ObstacleTypes.TrafficVehicle)
                {
                    ...
                    {
                        Debug.LogWarning(_closestObstacle.Name+" is a vehicle but does not implement the ITrafficParticipant interface. If this is your player, attach the PlayerComponent or your own implementation of the interface.", _closestObstacle.Collider);
