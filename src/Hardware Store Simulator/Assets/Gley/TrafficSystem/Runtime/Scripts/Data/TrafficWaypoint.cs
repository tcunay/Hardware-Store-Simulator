...
namespace Gley.TrafficSystem
{
    ...
    public class TrafficWaypoint : Waypoint
    {
        ...
        {
            // check existence first (no LINQ)
            for (int i = 0; i < _otherLanes.Length; i++)
            {
                if (_otherLanes[i] == waypointIndex)
                    return;
            }

            int[] newArray = new int[_otherLanes.Length + 1];

            for (int i = 0; i < _otherLanes.Length; i++)
            {
                newArray[i] = _otherLanes[i];
            }

            newArray[newArray.Length - 1] = waypointIndex;

            _otherLanes = newArray;
        }
    }
}