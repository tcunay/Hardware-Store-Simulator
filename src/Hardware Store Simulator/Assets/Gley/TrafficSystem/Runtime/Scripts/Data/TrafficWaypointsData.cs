...
namespace Gley.TrafficSystem
{
    ...
    public class TrafficWaypointsData : MonoBehaviour
    {
        ...
        public List<NeighborStruct> GetNeighborsWithConditions(int waypointIndex, VehicleTypes vehicleType)
        {
            ...
            {
                if (_allTrafficWaypoints[allNeighbors[i]].TemporaryDisabled)
                    continue;

                var allowedVehicles = _allTrafficWaypoints[allNeighbors[i]].AllowedVehicles;
                for (int j = 0; j < allowedVehicles.Length; j++)
                {
                    if (allowedVehicles[j] == vehicleType)
                    {
        ...
        private bool IsWaypointIndexValid(int waypointIndex)
        {
            if (waypointIndex < 0)
            {
                Debug.LogError("Waypoint index " + waypointIndex.ToString() + " should be >= 0");
                return false;
            }

            if (waypointIndex >= _allTrafficWaypoints.Length)
            {
                Debug.LogError("Waypoint index " + waypointIndex.ToString() + " should be < " + _allTrafficWaypoints.Length.ToString());
                return false;
            }

            if (_allTrafficWaypoints[waypointIndex] == null)
            {
                Debug.LogError("Waypoint at " + waypointIndex.ToString() + " is null, Verify the setup");
                return false;
            }
