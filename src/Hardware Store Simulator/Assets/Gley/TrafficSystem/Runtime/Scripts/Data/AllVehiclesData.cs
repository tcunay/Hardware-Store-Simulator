...
namespace Gley.TrafficSystem
{
    ...
    public class AllVehiclesData
    {
        ...
        public bool IsVehicleIndexValid(int vehicleIndex)
        {
            if (vehicleIndex < 0)
            {
                Debug.LogError("Vehicle index " + vehicleIndex.ToString() + " should be >= 0");
                return false;
            }

            if (vehicleIndex >= _allVehicles.Length)
            {
                Debug.LogError("Vehicle index " + vehicleIndex.ToString() + " should be < {_allVehicles.Length}");
                return false;
            }

            if (_allVehicles[vehicleIndex] == null)
            {
                Debug.LogError("Vehicle at " + vehicleIndex.ToString() + " is null, Verify the setup");
                return false;
            }
            return true;
        }
