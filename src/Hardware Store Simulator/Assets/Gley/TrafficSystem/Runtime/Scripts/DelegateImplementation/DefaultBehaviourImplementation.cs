...
namespace Gley.TrafficSystem
{
    public class DefaultBehaviourImplementation : IBehaviourImplementation, IDestroyable
    {
        ...
        {
            ITrafficParticipant otherVehicle = closestObstacle.VehicleScript;
            if (otherVehicle == null)
            {
                Debug.LogWarning($"{closestObstacle.Collider.name} is configured as a player vehicle but does not implement the ITrafficParticipant interface. Attach the PlayerComponent or your own implementation of the interface.", closestObstacle.Collider);
                return;
            }
