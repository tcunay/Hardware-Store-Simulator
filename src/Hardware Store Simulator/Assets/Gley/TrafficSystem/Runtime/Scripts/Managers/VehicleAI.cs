...
namespace Gley.TrafficSystem
{
    public class VehicleAI : IDestroyable
    {
        ...
        public bool AddRandomWaypoint(int vehicleIndex, VehicleTypes vehicleType)
        {
            ...
            if (oldWaypointIndex == TrafficSystemConstants.INVALID_WAYPOINT_INDEX)
            {
                Debug.LogWarning($"Vehicle {vehicleIndex.ToString()} has no valid last waypoint.");
                return false;
            }
            ...
            // Nothing found
            Debug.LogWarning("No suitable neighbors or lanes found for waypoint "+oldWaypointIndex.ToString()+". Vehicle "+vehicleIndex.ToString()+" will stop.");
            return false;
        }
