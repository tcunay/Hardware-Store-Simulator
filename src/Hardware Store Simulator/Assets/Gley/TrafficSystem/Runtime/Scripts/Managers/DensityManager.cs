...
namespace Gley.TrafficSystem
{
    ...
    public class DensityManager
    {
        ...
        public bool RequestIgnoredVehicle(int vehicleIndex, Vector3 position, UnityAction<VehicleComponent, int> completeMethod)
        {
        ...
        /// </summary>
        public bool CancelIgnoredVehicleRequest(int vehicleIndex)
        {
            for (int i = _requestedVehicles.Count - 1; i >= 0; i--)
            {
                VehicleRequest request = _requestedVehicles[i];
                if (request.Category == Category.User && request.Vehicle != null &&
                    request.Vehicle.ListIndex == vehicleIndex)
                {
                    _requestedVehicles.RemoveAt(i);
                    return true;
                }
            }
