...
namespace Gley.TrafficSystem
{
    ...
    {
        private readonly Dictionary<System.Type, VehicleBehaviour>[] _supportedBehaviours; // all behaviours available for a vehicle
        private readonly Dictionary<System.Type, VehicleBehaviour>[] _activeBehaviours; // currently active behaviours for a vehicle 
        ...
        public BehaviourManager(int nrOfVehicles, IBehaviourList defaultBehaviours, TrafficWaypointsData trafficWaypointsData, AllVehiclesData allVehiclesData)
        {
            ...
            _activeBehaviours = new Dictionary<System.Type, VehicleBehaviour>[nrOfVehicles];
            _supportedBehaviours = new Dictionary<System.Type, VehicleBehaviour>[nrOfVehicles];
        ...
        public void SetVehicleBehaviours(int vehicleIndex, VehicleBehaviour[] vehicleBehaviours)
        {
            ...
            _supportedBehaviours[vehicleIndex] = new Dictionary<System.Type, VehicleBehaviour>();
            _activeBehaviours[vehicleIndex] = new Dictionary<System.Type, VehicleBehaviour>();
            for (int i = 0; i < vehicleBehaviours.Length; i++)
            {
                if (_supportedBehaviours[vehicleIndex].TryAdd(vehicleBehaviours[i].GetType(), vehicleBehaviours[i]))
                {
        ...
        public void StartBehaviour(int vehicleIndex, System.Type behaviourType, object[] parameters)
        {
            var behaviour = GetBehaviour(vehicleIndex, behaviourType);
            if (behaviour != null)
            {
                behaviour.Start();
            ...
            {
                Debug.LogError($"{behaviourType} not found");
            }
        }
        ...
        public void StopBehaviour(int vehicleIndex, System.Type behaviourType)
        {
            GetBehaviour(vehicleIndex, behaviourType)?.Stop();
        }
        ...
        public VehicleBehaviour GetBehaviour(int vehicleIndex, System.Type behaviourType)
        {

            if (_supportedBehaviours[vehicleIndex].TryGetValue(behaviourType, out var behaviour))
            {
                return behaviour;
            }

            Debug.LogWarning($"Behaviour {behaviourType} not found on vehicle index {vehicleIndex}");
            return null;
        }


        public Dictionary<System.Type, VehicleBehaviour>[] GetActiveBehaviours()
        {
        ...
        {
            if (_activeBehaviours[vehicleIndex].ContainsKey(behaviour.GetType()))
            {
        ...
        private void UpdateActiveBehaviours(int vehicleIndex)
        {
            ...
            {
                _activeBehaviours[vehicleIndex].Add(_behavioursToAdd[vehicleIndex][0].GetType(), _behavioursToAdd[vehicleIndex][0]);
                _behavioursToAdd[vehicleIndex].RemoveAt(0);
            }
            while (_behavioursToRemove[vehicleIndex].Count > 0)
            {
                _activeBehaviours[vehicleIndex].Remove(_behavioursToRemove[vehicleIndex][0].GetType());
