...
namespace Gley.TrafficSystem.Editor
{
    ...
    public class VehicleComponentEditor : UnityEditor.Editor
    {
        ...
        const string _vehicleHolderName = "VehicleHolder";
        ...
        private static bool TryGetVehicleBounds(VehicleComponent targetScript, out Bounds totalBounds)
        {
            ...
            {
                if(targetScript.shadowHolder!=null)
                {
                    if (renderer.transform.IsChildOf(targetScript.shadowHolder))
                    {
                        continue; // Skip renderers that are children of the shadow holder
                    }
                }
            ...
            foreach (Renderer renderer in renderers)
            {
                ...
                if (!isWheel)
                {
                    if (renderer.TryGetComponent<MeshFilter>(out var meshFilter))
                    {
                        ...
                        {
                            float meshSIze = meshFilter.sharedMesh.bounds.size.sqrMagnitude;
                            if (meshSIze > maxSize)
                            {
                                maxSize = meshSIze;
                                objectToAdd = renderer;
        ...
        private static void AssignLights(VehicleComponent targetScript)
        {
            ...
            VehicleLightsComponentV2 vehicleLightsV2 = targetScript.GetComponent<VehicleLightsComponentV2>();
            if (vehicleLightsV2)
            {
                AssignGameobjects(targetScript, ref vehicleLightsV2.frontLights, "FrontLights");
                AssignGameobjects(targetScript, ref vehicleLightsV2.reverseLights, "ReverseLights");
                AssignGameobjects(targetScript, ref vehicleLightsV2.rearLights, "RearLights");
                AssignGameobjects(targetScript, ref vehicleLightsV2.stopLights, "StopLights");
                AssignGameobjects(targetScript, ref vehicleLightsV2.blinkerLeft, "BlinkersLeft");
                AssignGameobjects(targetScript, ref vehicleLightsV2.blinkerRight, "BlinkersRight");
            }
        }

        private static void AssignGameobjects(VehicleComponent targetScript, ref GameObject[] objsToAssign, string objectName)
        {
            List<Transform> found = targetScript.transform.FindDeepChildren(objectName);
            if (found.Count == 0)
            {
                return;
            }

            // exclude any transform that is a child of another found transform
            objsToAssign = found
                .Where(t => !found.Any(other => other != t && t.IsChildOf(other)))
        ...
        private static void SetCarHolder(VehicleComponent targetScript, ref bool correct)
        {
            ...
            if(carHolder == null)
            {
                carHolder = targetScript.transform.Find(_vehicleHolderName);
            }
            if (carHolder == null)
            {
