...
namespace Gley.UrbanSystem
{
    public class AStar
    {
        ...
        /// Uses A* algorithm.
        /// </summary>
        public List<int> FindPath(int startNodeIndex, int targetNodeIndex, int pedestrianTypes, PathFindingWaypoint[] allPathFindingWaypoints)
        {
            ...
            while (openSet.Count > 0)
            {
                ...
                for (int i = 0; i < currentNode.Neighbours.Length; i++)
                {
                    ...
                    for (int j = 0; j < neighbour.AllowedAgents.Length; j++)
                    {
                        if (neighbour.AllowedAgents[j] == pedestrianTypes)
                        {
                            agentAllowed = true;
                            break;
                        }
